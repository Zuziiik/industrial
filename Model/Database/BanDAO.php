...
class BanDAO {
	...
	public static function selectByUserId($userId) {
		...
		$result = queryMysql("SELECT id_ban, user_id_user, ban_start, ban_end FROM ban WHERE user_id_user='$userId' ORDER BY ban_start DESC");
	...
	public static function selectAllCurrent() {
		$result = queryMysql("SELECT id_ban, user_id_user, ban_start, ban_end FROM ban WHERE NOW() BETWEEN ban_start AND ban_end  ORDER BY ban_start DESC");
		$n = mysql_num_rows($result);
		$bans = array();
		for ($i = 0; $i < $n; ++$i) {
			$row = mysql_fetch_row($result);
	...
	public static function selectAll() {
		$result = queryMysql("SELECT id_ban, user_id_user, ban_start, ban_end FROM ban  ORDER BY ban_start DESC");
		$n = mysql_num_rows($result);
		$bans = array();
		for ($i = 0; $i < $n; ++$i) {
			$row = mysql_fetch_row($result);
